...
<div class="search-page" :class="{'show': isShow}">
	<div class="search-page-header bd-bottom" :class="{'not-ios': !isIOS, 'opacity': !isShow}">
		...
		<div class="search-input">
			<input ref="input" type="search" name="search"  v-model="query" @keyup.prevent.stop="search($event)" placeholder="搜索产品">
			<i class="iconfont iconsousuo"></i>
			<i v-if="query.length" class="iconfont iconcancel" @click.prevent.stop="clear"></i>
		</div>
		<div v-if="isIOS" class="right" @click.prevent.stop="hide">
			取消
		</div>
		<div v-if="!isIOS" class="right" @click.prevent.stop="chooseItem(null,true)">
			搜索
		</div>
	</div>
	<div class="has-header" v-show="isShow">
		<div class="history" v-if="historyItems.length">
			...
			<div class="history-content">
				<a v-for="(item, idx) in historyItems" :key="idx" class="cell bd-bottom" @click="chooseItem(item,false)">
					{{item[displayProp]}}
				</a>
				<div class="no-item" v-if="!historyItems.length">
					暂无历史记录
				</div>
			</div>
